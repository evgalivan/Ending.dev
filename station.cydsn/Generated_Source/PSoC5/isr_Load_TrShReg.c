...
//    if( TransmitShiftReg_RET_FIFO_EMPTY == TransmitShiftReg_GetFIFOStatus(TransmitShiftReg_IN_FIFO)){
    ...
    */
    ...
    register uint8 tmp = StartTransmit_Read(); // перед изменением прочитать
    StartTransmit_Write(tmp|2); //на pin 00 признак входа в прерывание
    if ((tmp & 0x1) != 0){ //если признак передачи очищен, то не загружать
        while(count_to_send){
    ...
    if((TransmitShiftReg_SR_STATUS&8) != 0){ //empty
        tmp = 0;//сброс контрольного регистра Старт
        BitCounterEnc_ReadStatusRegister();
        isr_TransmitWordShift_ClearPending();
        isr_TransmitWordShift_Enable();
        //TransmitShiftReg_WriteData(0xffffffff);//запись последнего слова в СдигРег, чтобы последняя была единица.
        isr_Load_TrShReg_Disable();
    }
    else     BitCounterEnc_ReadStatusRegister();//очищаем аппаратную причину прерывания
    ...
    TransmitShiftReg_GetIntStatus();//сбрасываем аппаратную причину прерывания
    isr_Load_TrShReg_ClearPending();//очищаем контроллер прерывания
